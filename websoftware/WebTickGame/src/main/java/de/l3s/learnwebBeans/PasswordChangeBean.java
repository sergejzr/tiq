...
public class PasswordChangeBean extends ApplicationBean implements Serializable {
	 ...
	 * if(parameter == null || parameter.equals("")) parameter =
	 * getFacesContext().getExternalContext().getRequestParameterMap().get("u");
	 * 
	 * if(parameter == null) { addMessage(FacesMessage.SEVERITY_ERROR,
	 * "invalid_request"); return; }
	 * 
	 * String[] splits = parameter.split("_"); if(splits.length != 2) {
	 * addMessage(FacesMessage.SEVERITY_ERROR, "invalid_request"); return; } int
	 * userId = Integer.parseInt(splits[0]); String hash = splits[1];
	 * 
	 * user = getLearnweb().getUserManager().getUser(userId); if(null == user ||
	 * !hash.equals(PasswordBean.createHash(user))) {
	 * addMessage(FacesMessage.SEVERITY_ERROR, "invalid_request"); return; }
	 * 
	 * canChangePassword = true; }
	 * 
	 * public boolean canChangePassword() { return canChangePassword; }
	 */
